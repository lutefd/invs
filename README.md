...
The notebook inspects CVM filings in a separate one-to-many section using `filings_as_of(...)`; it never joins them into the price/fundamental snapshot. Set `EXAMPLE_FILING_MODE=installation_replay` only for installation-time replay. It also validates and summarizes an existing `market-basic` artifact, using `EXAMPLE_FEATURE_MANIFEST` when a specific manifest is required. CVM IPE rows use explicit receipt-time `available_at` and unknown publication precision for live replay, while CAD is a current issuer snapshot excluded from historical filing claims.
