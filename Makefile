...
migrate: setup config
	...
	@$(COMPOSE) exec -T postgres sh -c \
		'psql -U "$$POSTGRES_USER" -d "$$POSTGRES_DB" -Atc "SELECT to_regclass('"'"'public.discovery_indexes'"'"')"' | \
		grep -qx 'discovery_indexes' || \
		$(COMPOSE) exec -T postgres sh -c 'psql -U "$$POSTGRES_USER" -d "$$POSTGRES_DB" -v ON_ERROR_STOP=1' < migrations/000017_discovery_indexes.up.sql
...
market-cycle: setup config
	@scripts/market-cycle.sh
...
discovery-run: config
	...
	@test -n "$(DISCOVERY_MOMENTUM_MANIFEST)" || (echo "DISCOVERY_MOMENTUM_MANIFEST is required" >&2; exit 2)
	@test -n "$(DISCOVERY_MARKET_SESSION)" || (echo "DISCOVERY_MARKET_SESSION is required" >&2; exit 2)
	@scripts/discovery.sh "$(DISCOVERY_PROFILE)" "$(DISCOVERY_MOMENTUM_MANIFEST)" \
		"$(DISCOVERY_MARKET_SESSION)" "$(or $(DISCOVERY_OUTPUT_ROOT),/data/research/discovery/nasdaq-100-starter)"

discovery-validate: config
	@test -n "$(DISCOVERY_MANIFEST)" || (echo "DISCOVERY_MANIFEST is required" >&2; exit 2)
	@$(COMPOSE) run --rm --no-deps jupyter python -m research.discovery_cli validate \
...
v1-release-guard-acceptance:
	@scripts/test-v1-release-guard.sh
...
paper-make-target-acceptance:
	@scripts/test-paper-make-target.sh
...
v1-pre-release-acceptance: config
	...
	@$(MAKE) v1-release-guard-acceptance
	@$(MAKE) paper-make-target-acceptance
