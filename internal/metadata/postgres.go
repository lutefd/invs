...
type ProviderInputs struct {
	...
	B3HistoricalQuoteYear    int                            `json:"b3_historical_quote_year,omitempty"`
	...
	FXRequest                *FXRequestInput                `json:"fx_request,omitempty"`
...
type FXRequestInput struct {
	BaseCurrency   string `json:"base_currency"`
	QuoteCurrency  string `json:"quote_currency"`
	RateKind       string `json:"rate_kind"`
	FixingTimezone string `json:"fixing_timezone"`
...
type B3InstrumentInput struct {
	SecurityID string `json:"security_id"`
	Ticker     string `json:"ticker"`
	ISIN       string `json:"isin"`
}

type MembershipSecurityInput struct {
	SecurityID string `json:"security_id"`
	Ticker     string `json:"ticker"`
	MIC        string `json:"mic"`
}
...
var sources = []source{{"sec", "SEC EDGAR", "fundamentals", "https://data.sec.gov", true}, {"yahoo", "Yahoo Finance", "market_data", "https://query1.finance.yahoo.com", true}, {"fred", "Federal Reserve Economic Data", "macro", "https://fred.stlouisfed.org", true}, {"alfred", "Archival FRED", "macro", "https://api.stlouisfed.org/fred/", true}, {"bcb", "Banco Central do Brasil SGS", "macro", "https://api.bcb.gov.br/dados/serie/", true}, {"bcb_ptax", "Banco Central do Brasil PTAX", "fx", "https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/", true}, {"b3", "B3 Public Instruments, Calendars, and Index Notices", "security_master", "https://www.b3.com.br/", true}, {"b3_cotahist", "B3 Historical Quotes", "market_data", "https://bvmf.bmfbovespa.com.br/InstDados/SerHist/", true}, {"b3_calendar", "B3 Historical Calendar Artifacts", "market_calendar", "https://www.b3.com.br/data/files/", true}, {"nasdaq", "Nasdaq Index Notices", "universe_membership", "https://www.globenewswire.com/", true}, {"nasdaq_calendar", "Nasdaq Exchange Calendar", "market_calendar", "https://www.nasdaqtrader.com/Trader.aspx?id=Calendar", true}, {"nyse", "NYSE Exchange Calendar", "market_calendar", "https://www.nyse.com/trade/hours-calendars", true}, {"cvm", "CVM Dados Abertos", "filings", "https://dados.cvm.gov.br/dados/", true}}
...
func (r *Repository) SyncCatalog(ctx context.Context, cfg config.Config) error {
	...
	for _, s := range sources {
		enabled := map[string]bool{"sec": cfg.Providers.SEC.Enabled || cfg.Providers.SECActionHistory.Enabled, "yahoo": cfg.Providers.Prices.Enabled, "fred": cfg.Providers.FRED.Enabled, "alfred": cfg.Providers.ALFRED.Enabled, "bcb": cfg.Providers.BCB.Enabled, "bcb_ptax": cfg.Providers.PTAX.Enabled, "b3": cfg.Providers.B3.Enabled || cfg.Providers.B3Membership.Enabled || cfg.Providers.B3ListingHistory.Enabled || cfg.Providers.B3ActionReplay.Enabled, "b3_cotahist": cfg.Providers.B3HistoricalPrices.Enabled, "b3_calendar": cfg.Providers.B3CalendarHistory.Enabled, "nasdaq": cfg.Providers.NasdaqMembership.Enabled, "nasdaq_calendar": cfg.Providers.NasdaqCalendar.Enabled || cfg.Providers.NasdaqCalendarHistory.Enabled, "nyse": cfg.Providers.NYSE.Enabled, "cvm": cfg.Providers.CVM.Enabled}[s.code]
