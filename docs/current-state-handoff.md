...
and acceptance fixture. The explicit after-close paper decision clock landed in
`ee29219`: direct paper runs accept a canonical UTC `--decision-at`, the daily-cycle
schema requires and hashes the top-level cutoff, generated paper commands carry it,
and delayed valuation/risk lookups remain bounded by that cutoff while economic
...
release compatibility hashes. `58e1615` integrates the read-only paper-session input
...
`6d5edab` with 224 Python tests; the integrated
...
from `6d5edab`. The genuine-only `v1-release-acceptance` wrapper landed in
...
interruption/resume stages; the latest rerun is pinned to `6d5edab` and the
...
gate to 218 Python tests; the destination-preflight regression in `010cfc0` brought
...
evidence index records that latest result from `6d5edab`.
...
The paper and daily-cycle paths now support receipt-delayed close data without
looking ahead: `ee29219` binds an explicit after-close UTC cutoff into the daily-cycle
specification hash, generated paper command, decision/report identity, valuation,
and risk evidence. The durable cycle report also records the cutoff for operator
inspection. The default direct paper invocation still uses the session close,
and the complete cycle requires the cutoff so a resume cannot silently move its
information boundary. This improves readiness for a genuine session but does not
create one; no genuine wall-clock forward record exists yet.
...
The operator input gap was first covered by `3b1ad11`: the read-only
...
daily cycle. `58e1615` now inserts that same preflight as a required per-account
daily-cycle stage before account creation, so a bad paper input cannot create a
durable account or ledger through the resumable path.
...
live parser check against the official Nasdaq Trader holiday page. A real bounded
collector run then published all 365 XNAS 2026 sessions at `xnas_2026_0f89a915a6cc`,
with `issues=0` reconciliation and a validated post-run external backup; it does
not create a genuine paper session. Pinned after-close lookups crossed a normal
BVMF close and the XNYS Thanksgiving closure. These current/reference versions are
available only from local receipt time; the pages expose no historical
publication/correction chronology required to close the v0.2 calendar gate. See the
...
- Latest current XNAS calendar publication acceptance: [2026-08-30 XNAS publication report](acceptance/2026-08-30-nasdaq-calendar-publication.md)
...
- Corporate-action publication and adjustment artifacts are accepted for one exact
  ...
  BVMF/XNYS/XNAS current/reference publication and accepted exact-artifact XNAS/BVMF
