...
commit `742e5ae`. B3 public `InstrumentsConsolidatedFile` is now integrated for
exact current/reference identity and listing snapshots. Its
`TradgStartDt`/`TradgEndDt` values remain source-defined trading intervals and its
