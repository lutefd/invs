...
commit `742e5ae`. B3 market data is not yet integrated. A bounded Yahoo `.SA`
verification supports a candidate price bridge but failed the security-master
evidence gate: access terms, stable instrument identity, historical lifecycle,
fixtures, coverage, and availability policy are not sufficient for historical use.
Official B3 public data is the candidate source for Brazilian identity/listing
evidence; Yahoo remains a separate price-bridge candidate pending its own terms
review.
