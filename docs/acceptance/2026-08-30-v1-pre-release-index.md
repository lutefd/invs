...
`47034ae5f08f5222598f1742f1ecc07000bc8501`; the operational snapshot remains the
earlier host evidence cited below. The current full `make test` gate passed 217
Python tests after daily-cycle input, report, and stage-log path hardening.
...
| --- | --- | --- |
| `INVS_BIND_ADDRESS=127.0.0.1 make test` | Passed: 217 Python tests, 63 schemas, Go tests/vet, Ruff, release/security checks, backup fixture | Runtime output from 2026-08-30 |
...
| `INVS_BIND_ADDRESS=127.0.0.1 make v1-pre-release-acceptance` | Passed: 217 Python tests, operational checks, migration replay, 11 resilience steps across 7 scenarios, 6 installation-lifecycle stages, replay guard, and workflow reports with intentional `attention` | Runtime output from 2026-08-30 at `47034ae`; exact report hashes are in the linked acceptance reports |
