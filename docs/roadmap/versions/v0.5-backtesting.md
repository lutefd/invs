...
- [x] Confirm historical-truth and feature entry gates.
- [x] Define immutable experiment identity and execution clocks.
- [x] Implement the auditable daily event loop and accounting ledger.
- [x] Apply actions, FX, fees, spread, slippage, and delays.
- [x] Add boring baseline strategies.
- [x] Publish versioned metrics, attribution, and experiment comparisons.
- [x] Enforce development/validation/holdout boundaries and validate walk-forward declarations.
- [x] Pass deliberate lookahead and survivorship-bias failures.
- [x] Reproduce bounded US and Brazil baseline experiments.
...
The fixed daily baseline boundary is accepted. Rolling train/calibration/test
execution remains a later extension; v0.5 validates its immutable window
declarations without claiming a rolling calibration runner.
## Goal
...
The accepted v0.5 slice validates non-overlapping declared windows and the
development/validation/holdout partition boundary. It does not execute rolling
calibration; that remains outside this version's accepted runtime.

### 8. LEAN integration decision

The v0.5 decision, recorded in [ADR 0017](../../adr/0017-lean-adapter-boundary.md),
keeps the internal daily simulator canonical and does not add a LEAN runtime
dependency. A future bounded adapter may:
...
improves fidelity and proves semantic equivalence. Do not change canonical data or
feature contracts to mirror LEAN internals.
...
The exit criteria passed on 2026-08-29. See the
[v0.5 backtesting acceptance report](../../acceptance/2026-08-29-v0.5-backtesting.md)
for the retained five-experiment US/Brazil reproduction, metadata recovery drill,
checkpoint replay, and 13-probe bias audit.
