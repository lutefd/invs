...
decision/report identity and reject mismatched replays. The timing regressions
brought the full gate to 222 Python tests. The latest resilience,
installation-lifecycle, and composed pre-release reruns passed from
`d7c4b4acb9a0a08d7070a596d87a8333584406bb` with 224 Python tests; `713e8a0` also exposes the cutoff in
the durable daily-cycle report and rejects changed cutoff identities on resume; its
daily-cycle preflight also checks
...
versions those contracts as `1.2.0`.
The composed pre-release report set was rerun from `d7c4b4a`; the genuine-only
