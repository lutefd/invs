...
`47034ae`; the genuine-forward gate remains unchanged. Daily-cycle input, report,
and stage-log path boundaries now reject symlinks and report temporary-file races
under the same hardening commit. The forward capture boundary
...
current full `make test` gate passes 217 Python tests.
