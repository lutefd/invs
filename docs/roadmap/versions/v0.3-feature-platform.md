...
The accepted entry slice is intentionally bounded: the registry currently contains
only `market-basic` 1.0.0, whose price input is explicitly
`installation_replay_only` because its availability is receipt-time. The batch
manifest and child artifacts are reproducible and tamper-checked in focused tests;
the catalog registration revalidates those immutable objects and stores their
discovery/lineage envelope. The read-only report adds catalog-level partition coverage
and lineage inspection, but does not claim the v0.3 exit gate, feature-level null
quality, or broad historical feature fitness.
