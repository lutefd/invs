...
- [x] Add the first accepted market/risk feature set, `market-momentum` 1.0.0
  (`dd450b3`, `941c6b5`).
- [x] Add reviewed fundamental mappings and feature sets (`53fe4fe`, `4827537`,
  `eee7fe1`).
- [x] Add macro-state features from historical vintages (`4827537`, `eee7fe1`).
- [x] Explain coverage, nulls, rejects, freshness, and raw lineage (`54a00d2`,
  `0454d0e`).
- [x] Reproduce the accepted multi-asset feature dataset in a clean root
  (`5fc3783`, `0454d0e`).
...
The accepted v0.3 slice is intentionally bounded: the registry contains
`market-basic`, `market-momentum`, `fundamental-growth`, and `macro-state`, each at
1.0.0. The latter three are backed by explicit reviewed producers, taxonomy or
historical-vintage selectors, strict manifests, and typed null/rejection policies.
Receipt-time price inputs remain explicitly `installation_replay_only`; no feature
family upgrades source fitness. The read-only quality report applies each input's
decision clock before reporting feature coverage, null reasons, stale inputs,
rejects, source contribution, and raw locators. The acceptance fixture publishes
20 securities across two monthly decisions, resumes one prepublished child per
family, reproduces the full feature tree byte-for-byte in a clean root, crosses an
ALFRED revision boundary, and fails closed for five tamper classes.
...
- provides a read-only report of catalog-level coverage, per-decision unaccounted
  ...
  feature-level quality is provided by the separate filesystem report, while
  automatic orphan repair and independent child-artifact discovery remain outside
...
- The catalog report provides coverage by registered batch and decision date,
  unaccounted partition counts, accepted row counts, input fitness, and exact
  manifest/part lineage. The separate `invs-feature-quality report` path provides
  coverage by feature, registry-approved null reasons, stale inputs, rejected
  partitions, source contribution, and raw locators after applying decision clocks.
