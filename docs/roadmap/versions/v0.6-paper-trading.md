...
- [x] Confirm v0.5 baseline and bias-suite acceptance.
- [x] Construct deterministic target portfolios from signal artifacts.
- [x] Version and enforce risk policy outside strategy code.
- [x] Persist balanced, append-only paper account events.
- [x] Run idempotent daily proposals, approvals, fills, and reports.
- [x] Add paper portfolio and operational monitoring.
- [x] Rebuild all positions and NAV from the ledger.
- [x] Pass restart, duplicate, backup/restore, and reconciliation drills.
- [x] Retain a bounded recorded/replayed forward paper record.
...
The v0.6 gate is accepted at validation boundary `95a79b5`, with feature
implementation boundary `f02a57f`. The internal
paper engine keeps signal, target, risk, order, fill, valuation, and reconciliation
interfaces separate; uses close-to-next-open execution; supports manual and recorded
auto approval; and treats the local append-only ledger as the source of truth.

The deterministic acceptance covers 22 sessions across separate equal-weight and
momentum sleeves, five rebalances, no-op sessions, split/dividend/delisting actions,
restart and duplicate delivery, stale-data halting, risk rejection without orders,
exact rebuild, backup restore, and reconciliation. PostgreSQL stores an immutable
account/event envelope for operator reporting, and the `paper-portfolio` dashboard
exposes that projection without replacing the local ledger. The optional broker
sandbox was deliberately not used; real-money execution, leverage, shorting, margin,
and intraday behavior remain outside this version.

The retained evidence and exact command ladder are in the
[v0.6 acceptance report](../../acceptance/2026-08-29-v0.6-paper-trading.md).
## Goal
