...
- [x] Confirm v0.3 feature reproducibility and filing identity.
- [x] Publish the relational theme/evidence model.
- [x] Complete one reviewed AI-infrastructure reference theme.
- [x] Publish immutable document text-extraction artifacts.
- [x] Add narrow reviewed LLM-derived event proposals.
- [x] Add append-only hypothesis revisions and frozen predictions.
- [x] Build point-in-time evidence packs and research memo export.
- [x] Close and reproduce one complete hypothesis outcome loop.
...
The accepted implementation and evidence boundary is recorded in the
[v0.4 hypothesis-loop acceptance report](../../acceptance/2026-08-29-v0.4-hypothesis-loop.md)
at `3ac61e1`. The retained acceptance drill reconstructs the reviewed theme at its
decision date, rejects a future evidence reference, preserves human event review
history, blocks frozen-prediction mutation, and measures a later outcome from a
pinned artifact and policy.
## Goal
