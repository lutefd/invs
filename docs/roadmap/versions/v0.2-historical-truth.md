...
no live security-master or exchange-calendar source has been admitted, so the
historical listings/calendar checkboxes and v0.2 gate remain open.
...
therefore own the Brazil identity/listing path; Yahoo remains a candidate price bridge
only. This is a source-admission result, not a v0.2 completion claim.
