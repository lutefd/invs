...
the full historical listings/calendar checkboxes and v0.2 gate remain open.
...
evidence eligible only from local receipt time. Historical publication/effective/
correction chronology and the resulting US/Brazil bias audits remain open, so the
calendar tracker is intentionally unchecked. See the
