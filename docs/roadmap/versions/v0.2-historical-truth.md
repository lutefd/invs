...
**Source-selection discovery (2026-08-13):** Yahoo Finance is the primary B3
market-data bridge for the first Brazil research slice. Brazilian tickers use the
`.SA` suffix; the intended coverage is historical prices, volumes, dividends, splits,
and related market data. Selective B3 public datasets complement that bridge for
instrument metadata, delistings, corporate actions, and validation. This supports the
platform's medium- to long-term research, backtesting, simulation, and ML boundary
without making paid B3 credentials a prerequisite. The discovery is not an
implementation or historical-fitness acceptance.
...
- [ ] Validate the Yahoo-primary Brazil bridge and selective B3 public-data admission
  boundary.
